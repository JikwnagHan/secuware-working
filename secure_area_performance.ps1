...
function Copy-And-VerifyFile {
    ...
    if ($result.WriteOK -and $result.ReadOK -and $result.HashOK) {
        $result.Status = 'PASS'
    }
    else {
        $result.Status = 'FAIL'
...
function Test-SecureArea {
    ...
    foreach ($file in $files) {
        $record = Copy-And-VerifyFile -SourcePath $file.FullName -TargetRoot $SecureRoot -DatasetRoot $DatasetRoot
...
$deviceTypeInput = Read-RequiredValue -PromptText '저장장치 종류를 입력하세요 (예: 1, 2, 3, HDD, SSD, USB 등)'
$secureRoot = Read-RequiredValue -PromptText '대상 보안영역 폴더 경로를 입력하세요 (예: E:\\SecureArea)'
$datasetRoot = Read-RequiredValue -PromptText '샘플데이터를 생성/보관할 폴더 경로를 입력하세요'
$resultInput = Read-RequiredValue -PromptText '결과 데이터를 저장할 경로를 입력하세요 (폴더 또는 .xlsx 경로)'
...
$records = Test-SecureArea -DatasetRoot $datasetRoot -SecureRoot $secureRoot
...
$records | Export-Csv -Path $detailCsv -NoTypeInformation -Encoding UTF8
