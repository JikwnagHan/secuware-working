...
function Resolve-DriveRoot {
    ...
    Write-Host "[DEBUG] Raw drive input = '$Input'"
    ...
    $clean = (-join $allowedChars).Trim()
    $clean = $clean.ToUpper()

    if ([string]::IsNullOrWhiteSpace($clean)) {
        throw "드라이브 문자를 입력하지 않았습니다. (정규화 후에도 내용이 없음. 원래 입력: '$Input')"
    }
    ...
    if ($clean -notmatch '^[A-Z]:\\$') {
        throw "드라이브 형식이 올바르지 않습니다: '$Input' → 정규화 값: '$clean'"
    }
    ...
    if (-not (Test-Path -LiteralPath $clean -PathType Container)) {
        ...
        if (-not $psd) {
            throw "지정한 드라이브를 찾을 수 없습니다: '$clean' (원래 입력: '$Input')"
        }
    }
...
# Codex와 같이 Read-Host 다중 호출이 어려운 환경을 고려해 한 줄 입력을 우선 시도한다.
# 형식 예: 2;E:\SecureArea;E:\dataset;E:\logs
$rawInput = Read-Host '저장장치종류;보안영역경로;샘플데이터경로;결과경로 (예: 2;E:\SecureArea;E:\dataset;E:\logs) [Enter=개별 입력]'
...
if (-not [string]::IsNullOrWhiteSpace($rawInput)) {
    $parts = $rawInput -split '[;,]', 4
    if ($parts.Count -ge 1 -and -not [string]::IsNullOrWhiteSpace($parts[0])) {
        $deviceTypeInput = $parts[0].Trim()
    }
    if ($parts.Count -ge 2 -and -not [string]::IsNullOrWhiteSpace($parts[1])) {
        $secureRoot = $parts[1].Trim()
    }
    if ($parts.Count -ge 3 -and -not [string]::IsNullOrWhiteSpace($parts[2])) {
        $datasetRoot = $parts[2].Trim()
    }
    if ($parts.Count -ge 4 -and -not [string]::IsNullOrWhiteSpace($parts[3])) {
        $resultInput = $parts[3].Trim()
    }
}

if ([string]::IsNullOrWhiteSpace($deviceTypeInput)) {
    $deviceTypeInput = Read-RequiredValue -PromptText '저장장치 종류(예: 1=HDD, 2=SSD/NVMe, 3=USB, 4=ETC)'
}
...
if ([string]::IsNullOrWhiteSpace($resultInput)) {
    $defaultLogPath = Join-Path (Get-Location) 'logs'
    $resultInput = Read-Host "결과 데이터 저장 위치 (예: $defaultLogPath 또는 E:\\logs\\SecureArea_test_results.csv)"
    if ([string]::IsNullOrWhiteSpace($resultInput)) {
        $resultInput = $defaultLogPath
    }
}
