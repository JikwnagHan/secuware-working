...
$malwareGeneralText = if ($malwareGeneralPass) {
    "일반 영역(Intact {0}, Changed {1}, New {2}, Missing {3}) : 악성코드 시나리오 전반에서 파일이 온전히 유지되어 PASS(무결성 100%) 판정을 충족했습니다." -f $malwareGeneralRow.Intact, $malwareGeneralRow.Changed, $malwareGeneralRow.New, $malwareGeneralRow.Missing
} else {
...
$malwareSecureText = if ($malwareSecurePass) {
    "보안 영역(Intact {0}, Changed {1}, New {2}, Missing {3}) : 모든 파일이 변조 없이 유지되어 PASS 판정을 충족했으며, 보안 정책이 악성 행위를 효과적으로 차단했습니다." -f $malwareSecureRow.Intact, $malwareSecureRow.Changed, $malwareSecureRow.New, $malwareSecureRow.Missing
} else {
...
$secureChangeInsight = if ($malwareSecureRow.Changed -le 0 -and $malwareSecureRow.New -le 0) {
    '• 보안 영역은 악성 행위 시나리오에서도 추가 변조·신규 파일이 발생하지 않아 안정적으로 차단되었습니다.'
} else {
...
$analysisParagraphs = @(
    ...
    '',
    '종합 평가',
    '• 악성코드 대응: 보안 영역은 PASS 상태를 유지했지만 일반 영역의 결과를 참고해 변조·신규 파일이 발생한 기법을 추가적으로 점검해야 합니다.',
...
$overallConclusion = if ($malwareSecurePass -and $ransomSecurePass) {
    '• 보안 영역은 악성코드 및 랜섬웨어 평가 모두에서 90% 기준을 상회하는 PASS 결과를 기록했습니다.'
} else {
    '• 보안 영역은 일부 시나리오에서 보완이 필요하므로 CSV 세부 데이터를 기반으로 원인을 점검하세요.'
}

$generalConclusion = if ($malwareGeneralPass -and $ransomGeneralPass) {
    '• 일반 영역도 모든 시험에서 PASS 상태를 유지했습니다.'
} else {
    $malwarePassText = if ($malwareGeneralPass) { '예' } else { '아니오' }
    $ransomPassText = if ($ransomGeneralPass) { '예' } else { '아니오' }
    "• 일반 영역은 추가 보호가 필요합니다. (악성코드 PASS: $malwarePassText, 랜섬웨어 PASS: $ransomPassText)"
}

$finalAssessment = if (($malwareSecurePass -and $ransomSecurePass) -and ($malwareGeneralPass -and $ransomGeneralPass)) {
    '최종 판정: 전체 시험이 PASS 조건을 충족했습니다.'
} else {
    '최종 판정: 보안 영역은 PASS를 달성했으나 일반 영역 보호 수준을 추가로 강화하는 것이 좋습니다.'
}
