...
`io_performance_assessment.ps1`은 초기화된 저장장치의 일반영역과 보안영역에 대해 동일한 샘플 데이터를 10회 반복 저장/읽기하여 속도를 측정하고, 보안영역이 일반영역 대비 90% 이상의 성능을 유지하는지 판정하는 자동화 스크립트입니다.
...
2. 안내에 따라 다음 경로를 순서대로 입력합니다.
  ...
  - 샘플 데이터 위치 (예: `D:\Test\Dataset`) – 실행 시 기존 파일을 삭제하고 10종 문서·환경 샘플을 새로 생성합니다.
  - 결과 데이터 저장 위치 (폴더 또는 `.xlsx` 경로)
3. 스크립트가 일반/보안 영역을 초기화한 뒤 각 샘플에 대해 “일반 쓰기 → 보안 쓰기 → 일반 읽기 → 보안 읽기” 순서를 10회 반복 측정합니다.
