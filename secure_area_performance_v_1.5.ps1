...
<#!
    ...
    ------------------------------------------------------------
    본 스크립트는 지정한 디스크 드라이브에 보안 영역(Secure_Area 폴더)을 생성하고,
    동일 드라이브에 샘플 데이터를 초기화한 뒤 쓰기/읽기/무결성 검증을 수행하여
    쓰기/읽기/무결성 검증을 수행한 뒤 CSV, XLSX, DOCX 결과를 자동으로 생성합니다.

    사용 순서
    1. 저장장치 종류와 테스트할 디스크 드라이브, 결과 저장 위치를 입력합니다.
    2. 선택한 드라이브 아래에 Secure_Area 및 DataSet 폴더를 초기화하고 샘플 데이터를 채웁니다.
    3. 보안영역 폴더의 기존 내용을 정리하고 샘플을 복사합니다.
    4. 각 파일의 쓰기/읽기/해시를 검증하여 세부/요약 결과를 산출합니다.
    5. 누적 CSV에 결과를 추가하고, 최신 CSV/JSON/XLSX/DOCX 보고서를 생성합니다.
...
function Resolve-DriveRoot {
    param([string] $DriveText)

    Write-Host "[DEBUG] Raw drive input (before clean) = '$DriveText'"

    if ($null -eq $DriveText) {
        throw '드라이브 문자를 입력하지 않았습니다. (입력이 null 입니다)'
    }

    $clean = $DriveText -replace '[\u0000-\u001F\u007F-\u009F\u200B-\u200D\uFEFF]', ''
    $clean = $clean.Trim()

    $allowedChars = $clean.ToCharArray() | Where-Object {
        ($_ -ge 'A' -and $_ -le 'Z') -or
        ($_ -ge 'a' -and $_ -le 'z') -or
        $_ -eq ':' -or
        $_ -eq '\' -or
        $_ -eq '/'
    }
    $clean = (-join $allowedChars).Trim().ToUpper()

    if ([string]::IsNullOrWhiteSpace($clean)) {
        throw "드라이브 문자를 입력하지 않았습니다. (정규화 후에도 내용이 없음. 원래 입력: '$DriveText')"
    }

    if ($clean.Length -eq 1 -and $clean -match '^[A-Z]$') {
        $clean = $clean + ':'
    }
    ...
    if ($clean -notmatch '^[A-Z]:\$') {
        throw "드라이브 형식이 올바르지 않습니다: '$DriveText' → 정규화 값: '$clean'"
    }

    if (-not (Test-Path -LiteralPath $clean -PathType Container)) {
        $driveLetter = $clean.Substring(0, 2)
        $psd = Get-PSDrive -Name $driveLetter.TrimEnd(':') -ErrorAction SilentlyContinue
        if (-not $psd) {
            throw "지정한 드라이브를 찾을 수 없습니다: '$clean' (원래 입력: '$DriveText')"
        }
    }
...
#region 메인 실행 (Codex/로컬 겸용 버전)
Write-Host '=== 보안 영역 성능 평가 자동화 시작 ==='

# Codex 환경에서는 Read-Host를 여러 번 호출하면 빈 값이 전달되는 경우가 있어
# 한 번의 입력에서 세 값을 모두 받는 방식을 기본으로 사용한다.
# 형식 예시:
#   2;C:\;C:\logs
#   2,C:\,C:\logs
# 순서: [0] 저장장치종류 / [1] 드라이브 / [2] 결과경로

$rawInput = Read-Host '입력 (저장장치종류;드라이브;결과경로) 예: 2;C:\;C:\logs'
if ([string]::IsNullOrWhiteSpace($rawInput)) {
    $rawInput = '2;C:\;.\logs'
}

$normalizedInput = ($rawInput -replace '\p{C}', '').Trim()
if ([string]::IsNullOrWhiteSpace($normalizedInput)) {
    $normalizedInput = '2;C:\;.\logs'
}

$normalizedInput = $normalizedInput -replace ',', ';'

$segments = $normalizedInput.Split(';')
if ($segments.Length -gt 3) {
    $segments = @($segments[0], $segments[1], ($segments[2..($segments.Length - 1)] -join ';'))
}
while ($segments.Length -lt 3) {
    $segments += ''
}

$sanitize = { param([string] $value) (($value -replace '[\u0000-\u001F\u007F-\u009F\u200B-\u200D\uFEFF]', '').Trim()) }
$deviceTypeInput = if ($segments.Length -ge 1) { & $sanitize $segments[0] } else { '' }
$driveInput      = if ($segments.Length -ge 2) { & $sanitize $segments[1] } else { '' }
$resultInput     = if ($segments.Length -ge 3) { & $sanitize $segments[2] } else { '' }

if ([string]::IsNullOrWhiteSpace($deviceTypeInput)) { $deviceTypeInput = '2' }
if ([string]::IsNullOrWhiteSpace($driveInput))      { $driveInput      = 'C:\' }
if ([string]::IsNullOrWhiteSpace($resultInput))    { $resultInput     = '.\logs' }

Write-Host "[DEBUG] Parsed drive input (pre-normalize) = '$driveInput'"

if (-not [System.IO.Path]::IsPathRooted($resultInput)) {
    $resultInput = Join-Path (Get-Location) $resultInput
}

try {
    $driveRoot = Resolve-DriveRoot -DriveText $driveInput
}
catch {
    Write-Host "드라이브 입력을 확인할 수 없어 기본값 C:\ 로 진행합니다. 상세: $($_.Exception.Message)" -ForegroundColor Yellow
    $driveRoot = 'C:\'
}

$secureRoot  = Join-Path $driveRoot 'Secure_Area'
$datasetRoot = Join-Path $driveRoot 'DataSet'
...
Write-Host "[SecureArea] 테스트 경로: $secureRoot"
Write-Host "[Dataset] 초기화 경로: $datasetRoot"

$runStartTime = Get-Date
$runId        = $runStartTime.ToString('yyyyMMdd_HHmmss')
$seed         = Get-Random -Maximum 1000000

# ----- 이하 원본에 있던 나머지 실행 로직 그대로 유지 -----
# 1) 샘플 데이터 생성
...
# 3) 결과 저장 경로/CSV/XLSX/DOCX 생성 부분도 원본에 있던 그대로 아래에 둡니다.

$isCsvPath = $resultInput -match '\\.csv$'
if ($isCsvPath) {
    ...
    $cumulativeCsv = $resultInput
}
else {
    $reportDirectory = $resultInput
    Ensure-Directory -Path $reportDirectory
    $cumulativeCsv = Join-Path $reportDirectory 'SecureArea_TestResults.csv'
}
...
if (Test-Path -LiteralPath $cumulativeCsv) {
    $csvContent = $detailRows | ConvertTo-Csv -NoTypeInformation
    if ($csvContent.Count -gt 0) {
        $csvWithoutHeader = $csvContent | Select-Object -Skip 1
        if ($csvWithoutHeader.Count -gt 0) {
            Add-Content -LiteralPath $cumulativeCsv -Value $csvWithoutHeader
        }
    }
}
else {
    $detailRows | Export-Csv -Path $cumulativeCsv -NoTypeInformation -Encoding UTF8
}

$excelPath = Join-Path $reportDirectory "SecureArea_TestReport_${timestamp}.xlsx"
$sheetData = @(
...
Write-Host '--- 생성된 결과 ---'
Write-Host "누적 CSV : $cumulativeCsv"
