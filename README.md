...
`io_performance_assessment.ps1`은 동일한 샘플 데이터를 15회 반복해서 쓰고 읽어 일반영역과 보안영역의 저장소 성능을 비교·분석합니다. 측정 결과는 CSV·XLSX·DOCX 보고서로 출력되며, 보안영역 성능이 일반영역 대비 90% 이상 유지되는지 자동 판정합니다.
...
3. 두 영역을 모두 비우고 동일 파일을 저장/읽기 각각 15회 연속 수행하면서 시간과 MB/s를 측정합니다. 일반 영역 측정이 끝나면 즉시 보안 영역에서 동일한 절차를 반복합니다.
